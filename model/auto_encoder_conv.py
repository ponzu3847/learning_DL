...
from common.util import to_cpu,to_gpu,show_distribution,im2col,col2im
import matplotlib.pyplot as plt
import pickle

class Encoder:
    def __init__(self,input_dim,conv_param_list,output_size,generate_activation,activation,use_batchnorm,show_distribution):
        ...
        if generate_activation=='relu':
            weight_init_generate=2.0
        else:
            weight_init_generate=1.0
        ...
        #conv_param_list=[[FN,FS,pad,stride],...]
        self.layers=[]
        for i in range(len(conv_param_list)):
            filter_num=conv_param_list[i][0]
            filter_size=conv_param_list[i][1]
            pad=conv_param_list[i][2]
            stride=conv_param_list[i][3]
            
            pre_node_num=pre_channel_num*filter_size*filter_size    #前の層のノード数
            conv_W=rn(filter_num,pre_channel_num,filter_size,filter_size)*np.sqrt(weight_init_std/pre_node_num).astype('f')
            conv_b=np.zeros(filter_num).astype('f')
            
            self.layers.append(Convolution(conv_W,conv_b,pad,stride))
            pre_channel_num=conv_param_list[i][0]
            out_size=int((out_size+2*pad-filter_size)/stride+1)

            if use_batchnorm:
                gamma=np.ones(filter_num*out_size*out_size).astype('f')
                beta=np.zeros(filter_num*out_size*out_size).astype('f')
                self.layers.append(BatchNormalization(gamma=gamma,beta=beta))
            self.layers.append(activation_layer[activation]())
            if i%2==1:
                self.layers.append(Pooling(pool_h=2,pool_w=2,pad=0,stride=2))
                out_size//=2
                
        self.layers.append(GAP())
        affine_W=rn(filter_num,output_size)*np.sqrt(weight_init_generate/filter_num).astype('f')
        ...
        for layer in self.layers:
            if isinstance(layer,BatchNormalization):
                ...
                if self.show_distribution:
                    ...
                    show_distribution(x)
            x=layer.forward(x)
            if self.show_distribution and isinstance(layer,(Sigmoid,Relu,Tanh)):
                print(layer.__class__.__name__)
                show_distribution(x)
        return x
...
class ConvAE:
    def __init__(self,input_shape,conv_param_enc,generate_size,
                 hidden_list_dec,output_size,generate_activation='sigmoid',activation='relu',
                 use_batchnorm=False,show_distribution=False):
        self.encoder=Encoder(input_shape,conv_param_enc,generate_size,generate_activation,activation,use_batchnorm,show_distribution)
